...
#
# Key ideas:
#  - Keep model output = [field(4), sdf(1)] => (B,N,5)
...
#  - SDF supervision always comes from trunk_input[...,3] (d) after scaling.
#  - Adds zero-level anchor on |d_gt|<eps0 to improve surface quality.
#
...
def parse_arguments():
    ...
    parser.add_argument('--lambda_zero', type=float, default=5.0)
    parser.add_argument('--lambda_eik', type=float, default=0.05)
    parser.add_argument('--eps0', type=float, default=0.02)
...
def log_parameters(args, device, name):
    ...
    if args.phase == 'shape':
        ...
        logging.info(f'lambda_sdf={args.lambda_sdf}, lambda_zero={args.lambda_zero}, lambda_eik={args.lambda_eik}, eps0={args.eps0}')
...
def load_and_preprocess_data(args, experiment_dir):
    ...
    # sdf gt from trunk_input[...,3] (already scaled)
    d_tr = tr_tr[:, :, 3:4]
    d_te = tr_te[:, :, 3:4]

    y_train = np.concatenate([s_tr, d_tr], axis=2).astype(np.float32)  # (Ns,N,5)
    y_test = np.concatenate([s_te, d_te], axis=2).astype(np.float32)
...
class TripleCartesianProd(Data):
    ...
    def losses(self, targets, outputs, loss_fn, inputs, model, aux=None):
        """
        targets: (B,N,5) = field(4 with possible NaN) + sdf(1)
        outputs: (B,N,5)
        """
        self._step += 1
        args = self.args
        ...
        # zero-level anchor (surface quality): |d_gt|<eps0 -> d_pred^2
        eps0 = args.eps0
        m0 = torch.abs(d_gt) < eps0
        loss_zero = (d_pred[m0] ** 2).mean() if m0.any() else 0.0 * d_pred.mean()

        # eikonal (only in shape phase, keep light)
        loss_eik = 0.0 * d_pred.mean()
        if self.phase == 'shape' and args.lambda_eik > 0:
            B = targets.shape[0]
            Ne = 2048
        ...
        if self.phase == 'field':
            ...
            # shape-only by default (preserve field). If you want to "keep" field slightly:
            # total = 0.05*loss_field + args.lambda_sdf*loss_sdf + args.lambda_zero*loss_zero + args.lambda_eik*loss_eik
            total = args.lambda_sdf * loss_sdf + args.lambda_zero * loss_zero + args.lambda_eik * loss_eik
            lsf, lz, le = loss_sdf, loss_zero, loss_eik

        if (self._step % self._log_every) == 0:
            logging.info(
                f"[loss step {self._step}] phase={self.phase} total={float(total.detach().cpu()):.6e} | "
                f"field={float(loss_field.detach().cpu()):.3e} sdf={float(lsf.detach().cpu()):.3e} "
                f"zero={float(lz.detach().cpu()):.3e} eik={float(le.detach().cpu()):.3e} "
...
class ShapeDecoder(nn.Module):
    ...
    def forward(self, latent, trunk_xyz_encoded):
        ...
        sdf = torch.einsum("bh,bnhc->bnc", combined2, x_trunk) + self.b
        return sdf  # (B,N,1)
...
# =========================
# Minimal post-eval (overall field + sdf)
# =========================
def post_eval(model, output_scalers, experiment_dir, args):
    ...
    # -------- sdf metrics (always finite) --------
    trunk_scaler = pickle.load(open(f'{experiment_dir}/trunk_scaler.pkl', 'rb'))
    sdf_pred_scaled = test_outputs[:, :, 4]
    d_gt_scaled = model.data.test_x[2][:, :, 3]  # scaled d from inputs

    def inv_1d_feature(vals_scaled, scaler, col_index):
        lo, hi = scaler.feature_range
        data_min = scaler.data_min_[col_index]
        data_max = scaler.data_max_[col_index]
        scale = (hi - lo) / (data_max - data_min + 1e-12)
        return (vals_scaled - lo) / scale + data_min

    d_gt = inv_1d_feature(d_gt_scaled, trunk_scaler, 3)
    d_pred = inv_1d_feature(sdf_pred_scaled, trunk_scaler, 3)

    sdf_mae = np.mean(np.abs(d_pred - d_gt))
    sdf_rmse = np.sqrt(np.mean((d_pred - d_gt) ** 2))
    sdf_r2 = calculate_r2(d_gt.flatten(), d_pred.flatten())
    logging.info(f"[Overall SDF] MAE={sdf_mae:.6f}, RMSE={sdf_rmse:.6f}, R2={sdf_r2:.4f}")

    # band metric (surface relevance)
    eps = args.eps0
    band = np.abs(d_gt) < eps
    if band.mean() > 0:
        band_mae = np.mean(np.abs(d_pred[band] - d_gt[band]))
        band_rmse = np.sqrt(np.mean((d_pred[band] - d_gt[band]) ** 2))
        logging.info(f"[Band |d|<{eps}] MAE={band_mae:.6f}, RMSE={band_rmse:.6f}, ratio={band.mean():.4f}")
    else:
        logging.info(f"[Band |d|<{eps}] No samples in band.")
...
def main():
    ...
    name = (
        f'Changed2Phase_{args.phase}_RUN{args.RUN}_D{args.N_samples}_N{args.N_pt}_'
