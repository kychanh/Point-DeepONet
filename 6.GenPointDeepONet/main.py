...
# ============================
# Normalizing Flow (Design-NO code 그대로 포함)
# (You said you'll use the RealNVP code 그대로)
# ============================
try:
    ...
    try:
        ...
        # If you don't use NSF flows, you can ignore these imports.
        FunActivation = None
...
def parse_arguments():
    ...
    # NF hyperparams (added, default safe)
    parser.add_argument('--nf_flow_type', type=str, default='RealNVP', help='NF flow type (e.g., RealNVP)')
    parser.add_argument('--nf_num_flows', type=int, default=4, help='Number of NF layers')
    parser.add_argument('--nf_hidden_dim', type=int, default=64, help='NF hidden dim')
    parser.add_argument('--nf_activation', type=str, default='Tanh', help='NF activation (string for FunActivation)')
    parser.add_argument('--lambda_nf', type=float, default=1e-3, help='Weight for NF NLL loss')
...
def log_parameters(args, device, name):
    ...
    logging.info(f'NF flow: {args.nf_flow_type}, num_flows={args.nf_num_flows}, hidden={args.nf_hidden_dim}, act={args.nf_activation}')
    logging.info(f'lambda_nf={args.lambda_nf}\n\n')
...
class TripleCartesianProd(Data):
    ...
    def losses(self, targets, outputs, loss_fn, inputs, model, aux=None):
        ...
        inputs:
        inputs[1] = pointnet_input (B,Npt,3)
        inputs[2] = trunk_input (B,Npt,4=xyzd scaled)  # NOTE: we do NOT use d here anymore.
        """
        # -----------------------------
        # 0) unpack
        # -----------------------------
        ...
        d_gt = targets[:, :, 4]          # (B,N)
        u_pred = outputs[:, :, :4]
        d_pred = outputs[:, :, 4]        # (B,N)

        # -----------------------------
        # 1) field supervised (MSE)
        # -----------------------------
        loss_field = torch.mean((u_pred - u_gt) ** 2)

        # -----------------------------
        # 2) sdf supervised (SmoothL1)  <-- Gaussian NLL 폐기
        # -----------------------------
        # SmoothL1 is robust and works well with scaled targets
        loss_sdf_data = F.smooth_l1_loss(d_pred, d_gt)

        # -----------------------------
        # 3) eikonal constraint  ||∇ d(x)|| = 1  (GINN 근거)
        # -----------------------------
        # sample random points in scaled coord space [-1, 1]^3
        B = targets.shape[0]
        Ne = 2048  # keep fixed for minimal change; tune later if needed

        x_eik = (torch.rand(B, Ne, 3, device=outputs.device) * 2.0 - 1.0).requires_grad_(True)  # (B,Ne,3)
        x_eik_enc = model.net.trunk_encoding(x_eik.reshape(-1, 3)).reshape(B, Ne, -1)

        # IMPORTANT: use cached latent (avoid recomputing encoder)
        latent = model.net._cached_latent
        if latent is None:
            pointnet_input = inputs[1]
            latent = model.net.encoder(pointnet_input)

        # predict sdf at random points
        d_eik = model.net.shape_decoder(latent, x_eik_enc)  # (B,Ne,1)
        d_eik = d_eik.squeeze(-1)                           # (B,Ne)
        ...
        # -----------------------------
        # 5) weight schedule (minimal, stable)
        # -----------------------------
        # DeepXDE step tracking: we increment a counter in train_next_batch
        t = getattr(self, "_step", 0)

        # eikonal ramp: 0 -> 0.1 over first 5k steps
        eik_max = 0.1
        eik_warm = 1000
        eik_ramp = 5000
        if t < eik_warm:
            ...
            w_eik = min(eik_max, eik_max * (t - eik_warm) / max(1, (eik_ramp - eik_warm)))

        # nf ramp: 0 -> lambda_nf over 10k steps (NF는 초반에 켜면 자주 터짐)
        nf_max = float(self.lambda_nf)
        nf_warm = 5000
        nf_ramp = 15000
        if t < nf_warm:
            w_nf = 0.0
        else:
            w_nf = min(nf_max, nf_max * (t - nf_warm) / max(1, (nf_ramp - nf_warm)))

        # fixed weight for sdf data term (can tune later)
        w_sdf = 1.0

        total = loss_field + w_sdf * loss_sdf_data + w_eik * loss_eik + w_nf * loss_nf

        # (optional but strongly recommended) log components occasionally
        if t % 500 == 0:
            logging.info(
                f"[loss] step={t} field={loss_field.item():.4e} "
                f"sdf={loss_sdf_data.item():.4e} eik={loss_eik.item():.4e}(w={w_eik:.3g}) "
                f"nf={loss_nf.item():.4e}(w={w_nf:.3g}) total={total.item():.4e}"
            )
...
class DesignGenPointDeepONet(dde.maps.NN, nn.Module):
    ...
    def __init__(self, branch_condition_input_dim, pointnet_input_dim, trunk_input_dim,
                 ...
                 nf_flow_type="RealNVP", nf_num_flows=4, nf_hidden_dim=64, nf_activation="Tanh",
                 device_str="cuda:0"):
...
def define_model(args, device, data, output_scalers):
    ...
    model = DesignGenPointDeepONet(
        ...
        regularization=None,
        num_output_components=num_output_components,
        nf_flow_type=args.nf_flow_type,
...
def main():
    ...
    data = TripleCartesianProd(x_train, y_train, x_test, y_test, lambda_nf=args.lambda_nf)
