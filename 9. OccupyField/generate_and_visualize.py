...
class ShapeDecoder(nn.Module):
    ...
    def forward(self, latent, trunk_xyz_encoded):
        ...
        return occ_logit  # NO tanh (raw logits)
...
def visualize_generated_surfaces(
    ...
    out_dir,
    direction,
    component=3,            # 0:ux 1:uy 2:uz 3:vm 4:occ_prob
    num_samples=10,
    opacity=1.0,
    cmap="jet",
    use_clip=True,
...
def main():
    ...
    ap.add_argument("--iso", type=float, default=0.0, help="unused in occupancy mode")
    ...
    for i in range(args.num_samples):
        with torch.no_grad():
            ...
            occ_logit_np = occ_logit.squeeze(0).cpu().numpy().astype(np.float32)
            occ_prob_np = occ_prob.squeeze(0).cpu().numpy().astype(np.float32)
            occ_bin_np = (occ_prob_np >= 0.5).astype(np.uint8)
        ...
        grid_i.point_data["occ_logit"] = occ_logit_np
        grid_i.point_data["occ_prob"] = occ_prob_np
        grid_i.point_data["occ"] = occ_bin_np
        ...
        occ_mesh = grid_i.threshold(value=0.5, scalars="occ_prob")
        surf_path = os.path.join(args.out_dir, f"{args.case_key}_sample_{i:03d}_occ.vtk")
        occ_mesh.save(surf_path)
        surface_paths.append(surf_path)

        print(
            f"[OK] sample {i}: saved {grid_path} and {surf_path} | "
            f"occ_prob_min={occ_prob_np.min():.4f}, occ_prob_max={occ_prob_np.max():.4f}, "
            f"occupied_ratio={occ_bin_np.mean():.4f}"
        )
