...
def softmax_max(x, temperature=30.0, dim=-1):
...
def compute_inverse_loss(
    ...
    Shape:
      latent      = nf_inverse_from_z(z)
      occ_logit   = shape_decoder(latent, trunk_xyz_encoded)
      field_scaled= field_decoder(branch_condition_input, latent, trunk_xyz_encoded)

    Objective:
      soft stress inside shape + volume penalty + z regularization

    Monitoring:
      hard-thresholded max stress also reported
    """
    ...
    field_phys = inverse_minmax_transform_torch(field_scaled, output_scaler)
    vm_phys = field_phys[..., 3]
    
    # hard mask for monitoring
    mask_hard = (occ_prob >= hard_threshold).float()
    vm_inside_hard = vm_phys * mask_hard                                   # (1,N)

    # soft mask for optimization
    vm_inside_soft = vm_phys * occ_prob
    #stress_obj = torch.max(vm_inside_hard)#softmax_max(vm_inside_soft, temperature=stress_temperature, dim=1).mean()
    stress_obj = torch.mean(vm_inside_hard)

    vol_frac = mask_hard.mean()
    vol_obj = (vol_frac - target_vol_frac) ** 2

    z_reg = (z ** 2).mean()

    total = (
        lambda_stress * stress_obj
        + lambda_vol * vol_obj
        + lambda_z * z_reg
    )
    ...
    stats = {
        ...
        "stress_obj_soft": float(stress_obj.detach().cpu()),
        "vol_frac_soft": float(vol_frac.detach().cpu()),
        "vol_obj": float(vol_obj.detach().cpu()),
        "z_reg": float(z_reg.detach().cpu()),
        "vm_max_soft_inside": float(vm_inside_soft.max().detach().cpu()),
        "vm_mean_soft_inside": float(vm_inside_soft.mean().detach().cpu()),
        "vm_max_hard_inside": float(vm_inside_hard.max().detach().cpu()),
    ...
    aux = {
        ...
        "vm_phys": vm_phys.detach(),
        "vm_inside_soft": vm_inside_soft.detach(),
        "vm_inside_hard": vm_inside_hard.detach(),
    }
...
def make_snapshot(it, z, stats, aux):
    return {
        ...
        "vm_phys": aux["vm_phys"].detach().cpu().numpy().copy(),
        "vm_inside_soft": aux["vm_inside_soft"].detach().cpu().numpy().copy(),
        "vm_inside_hard": aux["vm_inside_hard"].detach().cpu().numpy().copy(),
        "stats": dict(stats),
    }
    ...
    for it in range(1, n_iters + 1):
        ...
        if it == 1 or it % print_every == 0 or it == n_iters:
            print(
                ...
                f"stress_soft={stats['stress_obj_soft']:.6f} | "
                f"vm_hard_max={stats['vm_max_hard_inside']:.6f} | "
                f"vol_soft={stats['vol_frac_soft']:.6f} | "
...
def parse_args():
    ...
    parser.add_argument("--case_key", type=str, required=True,
                        help="case key existing in xyzdmlc.npz key 'c'")
    ...
    parser.add_argument("--target_vol_frac", type=float, default=0.18)
    parser.add_argument("--lambda_stress", type=float, default=1.0)
    parser.add_argument("--lambda_vol", type=float, default=50.0)
    parser.add_argument("--lambda_z", type=float, default=1e-3)
    parser.add_argument("--stress_temperature", type=float, default=30.0)
    ...
    parser.add_argument("--save_name", type=str, default="inverse_best.npz")
...
def main():
    ...
    # load branch condition from case_key (mlc only)
    ...
    for r in range(args.n_restarts):
        ...
        best, trajectory = run_single_restart(
            ...
            trunk_xyz_encoded=trunk_xyz_encoded,
            output_scaler=output_scaler,
            target_vol_frac=args.target_vol_frac,
            ...
            stress_temperature=args.stress_temperature,
        ...
        traj_path = os.path.join(args.save_dir, f"trajectory_restart_{r+1}.pkl")
    ...
    np.savez_compressed(
        ...
        vm_phys=global_best["aux"]["vm_phys"].detach().cpu().numpy(),
        vm_inside_soft=global_best["aux"]["vm_inside_soft"].detach().cpu().numpy(),
        vm_inside_hard=global_best["aux"]["vm_inside_hard"].detach().cpu().numpy(),
